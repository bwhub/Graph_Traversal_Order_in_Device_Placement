...
class Coordinator(object):
        ...
        for i in range(n_agents):
            ...
            if c['remote_async_start_ports'] is not None:
                ...
                for j, p in enumerate(c['remote_async_start_ports']):
                    c['remote_async_start_ports'][j] = p + \
                        c['remote_async_n_sims'][j] * i
    ...
    def handle_baselines(self):
        ...
        while True:
            center_to_bl_collections = copy.deepcopy(
                center_to_bl_collections_mold)
            ...
            for cent, bl_collect in center_to_bl_collections.items():
                center_to_bl_collections[cent] = np.mean(
                    bl_collect, axis=0, dtype=np.float64)
    ...
    def init_summary(self):
        config = self.config
        summ_names = ['run_times/episode_end_rt', 'run_times/best_so_far',
                      'run_times/best_rew_rt', 'ent/tanhc_const', 'run_times/ep_best_rt']
        ...
        self.avg_aggs = set(['run_times/episode_end_rt', 'loss/loss', 'opt/logits', 'opt/lr', 'rew/reward', 'rew/baseline', 'rew/advantage', 'loss/log_probs',
                            'ent/entropy', 'opt/grad_norm', 'ent/ent_dec', 'opt/pre_sync_grad_norm', 'ent/tanhc_const', 'mem/mem_util', 'run_times/ep_best_rt', 'run_times/rew_rt'])

        self.min_aggs = set(['run_times/best_so_far', 'run_times/best_rew_rt',
                            'run_times/argmax_ep_best_rt',  'mem/best_mem_util_so_far'])
