...
from model.graph_traversal_order import GraphTraversalOrder
...
class NodeEmbeddings(object):
    ...
    def get_embedding(self):
        ...
        l = [self.cost, self.out_size, self.done_bit,
             self.curr_bit, self.start_time]
...
# TODO: check what this class does.
...
class ProgressiveGraph(object):
    ...
    def __init__(self, G, n_devs, node_traversal_order, seed=42):
        ...
        random.seed(seed)
        G = copy.deepcopy(G)  # Ooops, might be causing memory leak?
        self.G = G
        self.n_devs = n_devs
        NodeEmbeddings.n_devs = n_devs
        self.seed = seed

        # TODO check if this line decides the traversal order?
        if node_traversal_order == 'topo':
            # self.node_traversal_order = list(nx.topological_sort(self.G))
            self.node_traversal_order = GraphTraversalOrder.topological_sort(self.G)
        ...
        print('First ten nodes to traverse are {}'.format(self.node_traversal_order[:10]))
    ...
    def init_node_embeddings(self):
        ...
        for n in self.nodes():
            e = NodeEmbeddings(
                G.nodes[n]['cost'], G.nodes[n]['out_size'], G.nodes[n]['mem'])
            E.append(e)
    ...
    def update_start_times(self, start_times):
        for i, n in enumerate(self.nodes()):
            # TODO
            self.node_embeddings[i].update_start_time(
                start_times[self.node_embeddings[i].placement])
    ...
    def set_start_times(self, d):
        for i in range(self.n_nodes()):
            ...
            # TODO
            self.node_embeddings[i].update_start_time(
                d[self.node_embeddings[i].placement])
    ...
    def get_sorted_nodes(self, order):
        """Return a list of nodes sorted by the order given."""
        if order == 'topo':
            sorted_nodes = GraphTraversalOrder.topological_sort(self.G)
        elif order == 'lexico_topo':
            sorted_nodes = GraphTraversalOrder.lexicographical_topological_sort(self.G)
        elif order == 'reversed_topo':
            sorted_nodes = GraphTraversalOrder.reverse_topological_sort(self.G)
        elif order == 'alpha':
            sorted_nodes = GraphTraversalOrder.alpha_sort(self.G)
        elif order == 'dfs_preorder':
            sorted_nodes = GraphTraversalOrder.dfs_preorder_sort(self.G)
        elif order == 'dfs_postorder':
            sorted_nodes = GraphTraversalOrder.dfs_postorder_sort(self.G)
        elif order == 'bfs':
            sorted_nodes = GraphTraversalOrder.bfs_sort(self.G)
        elif order == 'random':
            sorted_nodes = list(self.G.nodes())
