...
def glorot(shape, scope='default', dtype=tf.float32):
    """Return a new variable with value initial_value which is determined by 
...
def zero_init(shape, scope='default', dtype=tf.float32):
    """Return a new variable with value 0."""
    with tf.variable_scope(scope):
        init = np.zeros(shape)
        return tf.Variable(init, dtype=dtype)
...
class SingleLayerFNN(object):
    """Class that represents a fully connected neural network (FNN) with a single layer."""
...
class FNN(object):
    """Class that represents a fully connected neural network (FNN) with a couple of hidden layers.

    Relu activation is applied to every but the final layer of the FNN.
    
    Variables:
    - inp_size: FNN input size;
    - hidden_laye_sizes: list of hidden layer sizes;
    ...
    def __init__(self, inp_size, hidden_layer_sizes, out_size, name, dtype=tf.float32):
        ...
        self.layers = layers # What is goind on here???
