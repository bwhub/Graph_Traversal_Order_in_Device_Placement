# This seems to be the file where we control the order of iteration.
...
class ProgressivePlacer:
    """This is ProgressivePlacer.

    'class ProgressivePlacer:' is the same as 'class ProgressivePlacer(object):' in Python3.

    """

    # is_supervised = False       # BW: I think this variable is never used?
    '''
    ...
    def place(self, G, n_devs, nn_model, sim_eval, config, pptfitem=None):
        ...
        print('Executing place() in ProgressivePlacer.')
        ...
        # What does this variable? Default seems to be 1.
        ...
        self.m_save_path = '%smodels/tf-models/%s' % (
            config['model_folder_prefix'], config['name'])
        # 'models/tf-models/test-2'  #
        ...
        self.profiling_chrome_trace = '%smodels/chrome-traces/%s/' % (
            config['model_folder_prefix'], config['name'])
        ...
        self.model = nn_model(
                                self.progressive_graphs[0].get_emb_size(),
        ...
        if not self.summ_send_q or self.log_tb_workers:
            ...
            if not config['dont_sim_mem']:
                summ_names += ['run_times/rew_rt',
                               'mem/mem_util', 'mem/best_mem_util_so_far']
        ...
        self.save_saver = tf.train.Saver(
            max_to_keep=100, keep_checkpoint_every_n_hours=2)
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            # BW: definition of order
            print('Disamb_nodes = {}'.format(disamb_nodes))
            nodes = [n for n in self.progressive_graphs[0].nodes() if n not in disamb_nodes]
            # sorted_nodes_order = 'alpha'
            sorted_nodes_order = config['placement_traversal_order']
            print('Placement traversal order is {}.'.format(sorted_nodes_order))
            sorted_nodes = [n for n in self.progressive_graphs[0].get_sorted_nodes(order=sorted_nodes_order) if n not in disamb_nodes]

            # ep: episode index
            # n_episodes: number of training episodes for each neural network
            for ep in range(n_episodes):
                # Whether to renitialize the Neural Network model for each episode.
                if config['reinit_model'] and ep > 1:
                    ...
                    self.restore_saver.restore(
                        sess, 'model/models/test-' + str(ep - 1))
                    print("Finished reinitializing NN model", datetime.now())

                # Set up the initial placement at the beginning of each episode.
                if config['vary_init_state']:
                    init_pl = self.progressive_graphs[0].get_random_placement(seed=ep)
                    # print('Random placement is {}'.format(init_pl))
                    print('Varying initial placement using seed {}.'.format(ep))
                ...
                self.init_pl = init_pl  # Initial placement at the beginning of each episode.
                ...
                # Run the episode with sorted nodes.    
                print('Start run_episode() for episode {}'.format(ep))
                _, epbest_run_time, epbest_mem_util, run_times, mem_utils, states, explor_acts, pls = \
                    self.run_episode(sess, init_pl, sorted_nodes, is_eval_ep, ep, config)

                batched_cum_rewards = []
                rew_rts = []
                # What is this run_times variable?
                ...
                if is_eval_ep:
                    ...
                    self.plot_eval_stats(
                        ep, run_times, mem_utils, rew_rts, self.init_pl, ep_best_pl)
                    if is_save_ep:
                ...
                if supervised:
                    summ = self.improve_supervised_placement(
                        sess, states, batched_cum_rewards, baselines, ep)
                else:
                    summ = self.improve_placement(
                        sess, states, batched_cum_rewards, baselines, ep)
    ...
    def tb_write(self, d, ep, eval=False):
        ...
        with open(self.tb_dir + '/summary' + str(ep).zfill(2) + '.json', 'w') as f:
            json.dump(str(d), f)
        # if not self.summ_send_q or self.log_tb_workers:
        #     if eval:
    ...
    def send_is_eval_on_transfer(self, q, rs):
        zero_rt, _, mem_util = self.eval_placement(
            self.progressive_graphs[0].get_zero_placement())
    ...
    def save_model(self, sess, ep):
        ...
        path = self.save_saver.save(
            sess, self.m_save_path, global_step=ep, write_meta_graph=False)
    ...
    def record_pl(self, ep):
        ...
        for (r, ep) in self.best_runtimes:
            ...
            for k, v in pl.items():
                pl[k] = int(v)
            j.append({'runtime': r,
                      'len(self.best_runtimes)': len(self.best_runtimes),
                      'pl': pl,
                      'ep': int(ep),
                      'is_eval_ep': int(is_eval_ep)
                      })

        with open(self.record_best_pl_file + '_' + str(ep).zfill(2) + '.json', 'w') as f:
            json.dump(j, f)

    def run_episode(self, sess, init_pl, nodes, is_eval_ep, ep, config):
        """Run one episode of training.
        
        This function run one episode of training where the RL agent traverse all the nodes in the given neural network.

        Parameters:
        - sess: A TensorFlow 1.X session object for running TensorFlow operations.
        - init_pl: Initial placement that will be set at the beginning of each episode.
        - nodes: Node name list that defines the graph traversal order.
        - is_eval_ep: If the current episode is an evaluation episode.
        - ep: epsidoe index
        ...
        """
        print('BW: Start executing run_episode() in progressive_placer.py')
        ...
        # Save the placement as a figure.
        if (1 + ep) % self.print_freq == 0:
            ...
            # title = 'Graph {}  Episode: {:>3}'.format (config['dataset'], ep+1) 
            # ax.set_title(title, {'fontsize': 32, 'color': 'green'})
            # Gg = self.progressive_graphs[0].G
            # Original layout used in Placeto 
            # pos = nx.kamada_kawai_layout(Gg)
            # Layout that are better for visualizing neural network
            # pos = nx.drawing.nx_agraph.graphviz_layout(Gg, prog='dot', args="-Grankdir=LR")
            ...
            # placement_graph_img_location = '%s/graph-%d.pdf' % (self.fig_dir, ep+1)
            # placement_graph_img_location = '%s/graph-%d.png' % (self.fig_dir, ep+1)
        ...
        # print('### sorted_nodes ###')
        # for ind, x in enumerate(sorted_nodes):
        #     print('{}: {}'.format(ind, x))

        # The place for changing the order of the code?
        for i in range(self.max_rounds):
        # TODO change back to the previous ones.
        # for i in range(10):
            is_last_rnd = (i == self.max_rounds - 1)

            # Original Method used in Placeto, iterate through index 0 -> len(nodes)-1
            node = nodes[i % len(nodes)]
            # print('The first ten nodes are {}'.format(nodes[:10]))

            print('{} : cur node is {}'.format(i, node))

            for pg in self.progressive_graphs:
                pg.set_cur_node(node)

            s2 = time()
            d, lo, feed, expl, train_outs = self.get_improvement(sess, node, start_times, is_eval_ep)
            nn_time += (time() - s2)
    ...
    def compute_rewards(self, run_times, mem_utils, config):
        ...
        if config['mem_penalty'] > 0:
            for i, mem_util in enumerate(mem_utils):
                ...
                run_times[i] = min(
                    run_times[i], config['max_runtime_mem_penalized'])
    ...
    def log_config(self, config):
        """Log the key value pairs that are not Queue in 'config' dictionary to config.txt in 'self.eval_dir'."""
        jsonable_config = {}
    ...
    def eval_placement(self, pl=None, asynch=None, retreive=None):
        """Evaluate placement using simulator.
        
        Parameters:
        - pl: a single placement to evaluate. 
                E.g., {'child/layer_6/final_combine/concat/concat': 0, ... }
                If None, the placement with self.progressive_graphs will be used.
        - asynch: 
        - retreiver:

        Returns:
        - run_times: runtime of the placement.
        - start_times:
        - mem_utils: Memory utilization.
        ...
        if asynch is not None:
            ...
            for pg, pl in zip(self.progressive_graphs, pls):
                ...
                # print('### self.sim_eval is of type {}'.format(type(self.sim_eval)))
                # import inspect
                # print(str(inspect.signature(self.sim_eval)))
                # run_time = 0.424242424242              
                # TODO
        ...
        # print('### mem_utils = {}'.format(mem_util))
    ...
    def get_improvement(self, sess, node, start_times, is_eval_ep):
        ...
        feed = model.get_feed_dict(
            self.progressive_graphs, node, start_times, self.n_peers)
        if is_eval_ep:
        ...
        for rnd, [feed, sample, lo, train_outs] in enumerate(states):
            ...
            if self.dont_repeat_ff:
                ...
                if not np.all(np.abs(logprob_grad_outs[0] * adv + ent_grad_outs[0] - grads[0]) < 1e-3):
                    print(test_lp_loss, pl_ent_loss, adv, loss)
                    print(logprob_grad_outs[0] * adv,
                          ent_grad_outs[0], grads[0])
                    import pdb
                    pdb.set_trace()
                    ...
    ...
    def set_seeds(self, i=None):
        """Set seeds for numpy, tensorflow, and python random module."""
        if i is None:
            i = 0
        s = 42 + i
        np.random.seed(s)
