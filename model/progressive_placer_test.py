"""Entry point for Placeto experiment.

This module contains the class to help orchestrate the experiment.
"""
...
class ProgressivePlacerTest(object):
    ...
    def sim_reward(n_devs, sim, p, G):
        print('### Entering function sim_reward().')
        ...
        run_time, dft, du, tom, nt, start_times = sim.get_runtime(G, n_devs, costs, op_memories, p)

        assert tom >= 0
        run_time += l_fact * tom

        print('run_time is {}'.format(run_time))
        print('start_times is {}'.format(start_times))
    ...
    def sim_single_gpu(n_devs, sim, p, G):
        print('### Entering function sim_single_gpu().')
    ...
    def sim_neigh_placement(n_devs, sim, p, G):
        print('### Entering function sim_neigh_placement().')
    ...
    def test(self, config):
        graph = config['graph']         # Name of the synthetic graph, if None, load from a file.
        # print('Name of the graph is {}'.format(graph))
        N = config['graph_size']        # Size of the synthetic graph.
        n_devices = config['n_devs']    # Number of devices to place.
        m_name = config['m_name']
        # print('Config used is {}'.format(config))
        f = None
        sim = None
        ...
        # ------------ Step 1 ------------
        # Create a graph - either synthetic, or from a file
        if graph == 'chain':
            ...
            # This is not a very smart way of passing parameters.
            # [0] exists here because 'config['pickled_inp_file']' is a list of a single element.
            ...
            # PPTFItem is related to graph features.
            pptf = PPTFItem(
                            input_file,
                            n_devices,
        ...
        print('### if not f is {}'.format(not f))
        if not f:
        ...
        if config['eval'] is not None:
            print('config[\'eval\'] is not None')
            _, r, ss, p = pptf.eval(config['eval'])

            fname = 'models/chrome-traces/%s/timeline.json' % (config['name'])
            # timeline_to_json(ss, p, fname)
        else:
            # I think this is where the training process happens
            print('Start ProgressivePlacer().place()')
            print('The Graph used is of type {}.'.format(type(G)))
            print('Number of devices is {}.'.format(n_devices))
            
            ProgressivePlacer().place(
                                        G,
                                        n_devices,
                                        ProgressivePlacerTest.choose_model(m_name),
...
def parse_args(args):
    ...
    This function is a refactor of the original Placeto code to make it easier to test the code.
    It returns a two item tuple containing the populated namespace and the list of remaining argument strings.

    Parameters:
    - args: list of strings that represents input args to parse.
    ...
    # TODO Check if this is the place that determines the traversal order
    parser.add_argument('--node-traversal-order', default='topo', help='Options: topo, random')
    # Order of nodes for performing placement within a episode.
    parser.add_argument('--placement-traversal-order', default='topo', help='Options: topo, random')
    ...
    # 'args' contains hyperparameters of the experiment, should consider save it somewhere to compare different experiments.
...
if __name__ == '__main__':
    ...
    print('####### sys.argv is of type {}'.format(type(sys.argv)))
    print('####### sys.argv is {}'.format(sys.argv))

    args, unknown = parse_args(sys.argv[1:])

    # print('The arguments passed to program are: {}.'.format(args))
    # print('The unknown arguments are: {}.'.format(unknown))
    ...
    if args.n_workers > 1:
        ...
        print('Number of workers is bigger than 1.')
    else:
        start_time = time()
        if(args.__dict__['dataset_folder'] is not None):
            r = []
            # Iterating through all the neural network graphs specified in the dataset_folder.
            for root, dirs, files in os.walk(args.__dict__['dataset_folder']):
                for name in files:
                    ...
                    print('The input pickle file that will be used is {}.'.format(args.__dict__['pickled_inp_file']))
                    ProgressivePlacerTest().test(args.__dict__)
                    # r.append(os.path.join(root, name))

        # print('Dataset folder not specified.')
        # # parameter dataset is need for the code to work properly
        # args.__dict__['dataset'] = args.__dict__['pickled_inp_file'][0].split('/')[1]       
        # print('The input pickle file that will be used is {}.'.format(args.__dict__['pickled_inp_file']))
        # ProgressivePlacerTest().test(args.__dict__)
