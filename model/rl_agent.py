...
class ReinforceAgent(object):
    ...
    def __init__(self, seed, bs):
        ...
        if self.tanhc_init:
            ...
            self.tanhc_decay_func = tf.minimum(
                tanhc_decay_func, self.tanhc_max)
  ...
  '''
    ...
    def _build_train_ops(self,
        ...
        for i in range(self.bs):
            with tf.variable_scope('log_prob_grads'):
                grads_and_vars = opt.compute_gradients(
                    self.log_prob_loss[i], tf_variables)
        ...
        self.logprob_grad_outs = [
            [g for g, _ in b_grads[i] if g is not None] for i in range(self.bs)]
        ...
        self.ent_grad_outs = [[g for g, _ in b_grads2[i]
                               if g is not None] for i in range(self.bs)]
        ...
        if sample_ph:
            ...
            if self.eps_init > 0:
                ...
                sample = tf.cond(tf.reduce_sum(eps) < eps_thres,
                                 lambda seed=self.seed: tf.multinomial(
                                     tf.ones_like(logits), 1, seed=seed),
                                 lambda: sample)
